...
# Runs run_custom_conf_vm.sh inside a container with Azure CLI, azcopy, uplink
# and zstd, so the host only needs Docker. All arguments are passed through.
#
# Authentication:
...
for ((i = 0; i < ${#args[@]}; i++)); do
  case "${args[i]}" in
    ...
    --provider-config) PROVIDER_CONFIG="${args[i + 1]:-}" ;;
  esac
done
...
if [[ -n "$PROVIDER_CONFIG" ]]; then
  [[ -d "$PROVIDER_CONFIG" ]] || die "Provider config directory not found: ${PROVIDER_CONFIG}"
  provider_dir="$(cd "$PROVIDER_CONFIG" && pwd)"
  mounts+=(-v "${provider_dir}:${provider_dir}:ro")
fi
...
exec docker run --rm \
  ...
  "${SCRIPT_DIR}/run_custom_conf_vm.sh" "$@"
