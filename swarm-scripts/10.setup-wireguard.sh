...
DB_HOST="$DB_HOST" DB_PORT="$DB_PORT" DB_USER="$DB_USER" DB_NAME="$DB_NAME" \
  bash "$CLI" create ClusterPolicies "$CLUSTER_POLICY"

echo "Creating/Updating ClusterServices '$SERVICE_NAME'..."
# Keep legacy service id = SERVICE_NAME for wireguard
DB_HOST="$DB_HOST" DB_PORT="$DB_PORT" DB_USER="$DB_USER" DB_NAME="$DB_NAME" \
  bash "$CLI" create ClusterServices "$SERVICE_NAME" --name="$SERVICE_NAME" --cluster_policy="$CLUSTER_POLICY" --version="$SERVICE_VERSION" --location="$LOCATION_PATH"
