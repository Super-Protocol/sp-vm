...
# This script bootstraps the blockchain-observer-service into SwarmDB via mysql client.
...
#     /usr/local/lib/sp-swarm-services/apps/blockchain-observer-service
...
# Service descriptors
SERVICE_NAME=${SERVICE_NAME:-sp-blockchain-observer-service}
SERVICE_VERSION=${SERVICE_VERSION:-1.0.0}
CLUSTER_POLICY=${CLUSTER_POLICY:-sp-blockchain-observer-service}
CLUSTER_ID=${CLUSTER_ID:-blockchain-observer-service}
...
DB_HOST="$DB_HOST" DB_PORT="$DB_PORT" DB_USER="$DB_USER" DB_NAME="$DB_NAME" \
  python3 "$(dirname "$0")/swarm-cli.py" create ClusterServices "$SERVICE_PK" --name="$SERVICE_NAME" --cluster_policy="$CLUSTER_POLICY" --minSize=1 --maxSize=1 --version="$SERVICE_VERSION" --location="$LOCATION_PATH" --omit-command-init
