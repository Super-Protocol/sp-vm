...
SERVICE_UNIT = "sp-svc-version-certificates-service.service"
PROPERTY_PREFIX = "version_certificates_service"
DEFAULT_PORT = 3005
...
def ensure_config_written(
    ...
    """
    Write configuration file for version-certificates-service using Jinja template.
    ...
    template_dir = Path("/etc/sp-swarm-services/templates/version-certificates-service")
...
def handle_apply(input_data: PluginInput) -> PluginOutput:
    ...
    try:
        ...
        config_path = Path("/etc/sp-swarm-services/apps/version-certificates-service/configuration.yaml")
