...
SERVICE_UNIT = "sp-svc-content-certificates-service.service"
PROPERTY_PREFIX = "content_certificates_service"
DEFAULT_PORT = 3004
DEFAULT_LOG_LEVEL = "info"
DEFAULT_RETRY_INTERVAL = 60000
DEFAULT_MAX_RETRIES = 5
DEFAULT_RETRY_INFO_TTL = 3600000
NATS_PORT = 4222
MONGODB_PORT = 27017
REDIS_PORT = 6379
...
def pick_redis_url(state_json: Dict[str, Any]) -> str:
    """
    Build Redis connection URL with all available Redis nodes' WG IPs.
    Format: redis://host1:port,redis://host2:port,...
    Falls back to localhost if none available.
    """
    redis_wg_props = state_json.get("redisWgNodeProperties") or []
    hosts = []
    for p in redis_wg_props:
        v = p.get("value")
        if v:
            hosts.append(f"redis://{v}:{REDIS_PORT}")
    if hosts:
        return ",".join(hosts)
...
def create_template_context(
    ...
    retry_info_ttl: int = DEFAULT_RETRY_INFO_TTL,
...
def ensure_config_written(
    ...
    """
    Write configuration file for content-certificates-service using Jinja template.
    ...
    template_dir = Path("/etc/sp-swarm-services/templates/content-certificates-service")
    ...
    context = create_template_context(
        ...
        retry_info_ttl=DEFAULT_RETRY_INFO_TTL,
...
def handle_apply(input_data: PluginInput) -> PluginOutput:
    ...
    try:
        ...
        config_path = Path("/etc/sp-swarm-services/apps/content-certificates-service/configuration.yaml")
