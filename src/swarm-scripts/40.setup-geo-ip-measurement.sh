...
# This script bootstraps the geo-ip-measurement service into SwarmDB via mysql client.
# Run it INSIDE the container. Assumes mysql client is available.
#
# Note:
# - The geo-ip-measurement manifest and main.py should be available inside the container at:
#     /etc/swarm-services/geo-ip-measurement/manifest.yaml and /etc/swarm-services/geo-ip-measurement/main.py
#   (mount or copy them similarly to the wireguard service)
#
# - geo-ip-measurement depends on a WireGuard cluster existing and sharing nodes with it.
#   When bootstrapping WireGuard, prefer ClusterPolicy id 'wireguard' to match geo-ip-measurement's stateExpr.
...
# Service descriptors
SERVICE_NAME=${SERVICE_NAME:-geo-ip-measurement}
SERVICE_VERSION=${SERVICE_VERSION:-1.0.0}
CLUSTER_POLICY=${CLUSTER_POLICY:-geo-ip-measurement}
CLUSTER_ID=${CLUSTER_ID:-geo-ip-measurement}
...
if DB_HOST="$DB_HOST" DB_PORT="$DB_PORT" DB_USER="$DB_USER" DB_NAME="$DB_NAME" \
  ...
  DB_HOST="$DB_HOST" DB_PORT="$DB_PORT" DB_USER="$DB_USER" DB_NAME="$DB_NAME" \
    python3 "$(dirname "$0")/swarm-cli.py" create ClusterPolicies "$CLUSTER_POLICY" --maxClusters=1
fi
